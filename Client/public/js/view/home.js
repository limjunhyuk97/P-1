...
export default class Home extends template {
  constructor(boardID, pageNumber, articleCount) {
    ...
    this.currentLeft =
      this.articleCount === 0
        ? 1
        : Math.floor((this.pageNumber - 1) / 6) * 6 + 1;
    this.currentRight =
      this.articleCount === 0
        ? 1
        : this.currentLeft + 5 > this.totalPage
        ? this.totalPage
        : this.currentLeft + 5;
  ...
  async getMain() {
    ...
    main_menu_articles.id = "main-menu-articles";
    main_menu_articles.innerHTML = `${this.boardName}`;
    main_menu_post.id = "main-menu-post";
    main_menu_post.innerHTML = `글쓰기`;
    ...
    await fetch(`/board/main?page=${this.pageNumber}&boardID=${this.boardID}`)
      ...
      .then((res) => {
        if (res.length === 0) {
          ...
          emptyMSG.classList.add("article-empty");
        ...
        res.forEach((el, idx) => {
          ...
          article.id = `article${idx + 1}`;
          const article_date = $.createElement("div");
          article_date.classList.add("article-date");
          article_date.innerHTML = `${el.date}`;
          const article_title = $.createElement("div");
          article_title.classList.add("article-title");
          article_title.innerHTML = `${el.title}`;
          const article_author = $.createElement("div");
          article_author.classList.add("article-author");
          article_author.innerHTML = `${el.author}`;
  ...
  async getFooter() {
    ...
    if (this.currentLeft > 1) {
      ...
      button.innerHTML = `<span class="material-icons md-24" data-link href='/home?board=${
        this.boardID
      }&page=${this.currentLeft - 1}'>navigate_before</span>`;
      buttons.push(button);
    }
    ...
    for (let i = this.currentLeft; i <= this.currentRight; ++i) {
      const button = $.createElement("div");
      button.classList.add("button");
      if (this.pageNumber == i) button.classList.add("current-btn");
    ...
    if (rightOn) {
      ...
      button.innerHTML = `<span class="material-icons md-24" data-link href='/home?board=${
        this.boardID
      }&page=${this.currentRight + 1}'">navigate_next</span>`;
      buttons.push(button);
    }
